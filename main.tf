...
resource "vault_generic_endpoint" "devops_team1_users" {
  for_each = {
    "user01" = {
      password = var.user1loginpass
      policies = ["devops_team1_policy"]
    }
    "user02" = {
      password = var.user2loginpass
      policies = ["devops_team1_policy"]
    }
  }
...
resource "vault_mount" "kvv2-team1" {
  path        = "kvv2-team1"
  type        = "kv"
  options     = { version = "2" }
...
resource "vault_kv_secret_v2" "devops-team1" {
  for_each = {
     "aws-master" = {
        accesskey             = "admin"
        secretaccessKey       = "xxxxee"

   }
   "aws-dev" = {
        access_key             = "admin"
        secret_access_Key       = "xxxxee"

   }
 } 
  mount     = vault_mount.kvv2-team1.path
  name      = each.key
...
resource "vault_policy" "devops_team1_policy" {
  ...
  policy = <<EOT
path "kvv2-team1/*" {
  capabilities = ["update", "delete", "read", "list"]
}
EOT
}
...
# # the below codes written by another way are also correct#
